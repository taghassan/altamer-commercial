...
class WPvivid_Dashboard
{
    ...
    public function init_plugin_install()
    {
        ...
        {
            update_option('wpvivid_plugin_install_cache',$plugin_install_cache);
            $ret['result']='success';
    ...
    public function addon_bar()
    {
        ...
        <div id="wpvivid_addons_list" style="padding: 0 1em 1em 1em;">
            <ul class="wpvivid-three-cols">
                ...
                if($this->has_backup_pro())
                {
                    $this->output_backup_addons($need_login);
                }
                else
                {
                    foreach ($plugins as $item)
    ...
    public function output_backup_addons($need_login)
    {
        $dashboard_info = get_option('wpvivid_dashboard_info', array());
        $plugins = $this->get_plugins_status($dashboard_info);
    ...
    public function addon_page_url($url,$slug)
    {
        ...
        {
            $url=apply_filters('wpvivid_white_label_page_redirect', 'admin.php?page=wpvivid-setting', 'wpvivid-setting').'&role_cap';
        }
        return $url;
    }
