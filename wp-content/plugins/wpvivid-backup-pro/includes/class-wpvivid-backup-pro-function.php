...
class WPvivid_backup_pro_function
{
    ...
    public static function is_wpvivid_other_backup($file_name)
    {
        ...
        else if(preg_match('/wpvivid-.*_.*backup_custom_othe.*\.zip$/',$file_name,$matches))
        {
            return true;
        }
        else if(preg_match('/'.apply_filters('wpvivid_white_label_file_prefix', 'wpvivid').'-.*_.*backup_custom_othe.*\.zip$/',$file_name,$matches))
        {
            return true;
        }
        else
