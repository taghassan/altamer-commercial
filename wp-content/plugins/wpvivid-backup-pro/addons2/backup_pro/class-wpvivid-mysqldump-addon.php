...
class WPvivid_Mysqldump_Addon
{
    ...
    {
        if(is_a($task, 'WPvivid_New_Backup_Task'))
        {
            $this->task=$task;
        }
        else
        ...
        $dumpSettingsDefault = array(
            ...
            'compress' => WPvivid_Mysqldump_Addon::NONE,
            'init_commands' => array(),
            'no-data' => array(),
            ...
            'default-character-set' => WPvivid_Mysqldump_Addon::UTF8,
            'disable-keys' => true,
        ...
        $this->dumpSettings['init_commands'][] = "SET NAMES " . WPvivid_Mysqldump_Addon::UTF8;
        ...
        $this->compressManager = new CompressTest();
    ...
    public function connect()
    {
        ...
        if(isset($this->dumpSettings['is_additional_db']))
        {
            if($dbType=='wpdb')
            {
                $this->typeAdapter=new WPvividTypeAdapterAdditionalWpdb(null);
            }
            else
            {
                ...
                {
                    $this->typeAdapter = TypeAdapterFactory::create($dbType, null);
                }
            }
        }
        else
    ...
    public function list_tables()
    {
        ...
        if(isset($this->dumpSettings['is_additional_db']))
        {
            $resultSet = $this->query('SHOW TABLE STATUS');
        }
        else
        {
            global $wpdb;
    ...
    public function check_tmp_file()
    {
        ...
        $max_backup_tables=$this->dumpSettings['max_backup_table'];
        if($max_file_size==0)
            return;
