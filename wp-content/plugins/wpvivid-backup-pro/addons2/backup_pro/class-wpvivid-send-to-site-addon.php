...
class WPvivid_Send_to_site_addon extends WPvivid_Remote_addon
{
    ...
    public function _upload($task_id, $file,$callback)
    {
        ...
        while (!feof($handle))
        {
            ...
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('send chunk failed.','warning');
                if($retry_count>3)
                {
                    ...
                    $wpvivid_plugin->wpvivid_log->WriteLog('upload failed, retry times: '.$retry_count,'warning');
                ...
                else
                {
                    ...
                    $wpvivid_plugin->wpvivid_log->WriteLog('reset offset: '.$offset,'warning');
                    if(fseek($handle, $offset)!==0)
                    {
                        $wpvivid_plugin->wpvivid_log->WriteLog('Seek file offset failed:'.size_format($offset),'notice');
                    }

                    $wpvivid_plugin->wpvivid_log->WriteLog('upload retry, retry times: '.$retry_count,'warning');
            ...
            if (isset($_POST['wpvivid_content'])) {
                ...
                if(isset($params['backup']['options']['backup_options']['backup']))
                {
                    ...
                    if(isset($backup_data['result']['files']))
                    {
                        ...
                        foreach ($files as $file)
                        {
                            $file_path=WP_CONTENT_DIR.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.$file['file_name'];
                            if(!file_exists($file_path))
                            {
                                $ret['result'] = WPVIVID_FAILED;
                ...
                if (isset($params['backup']) && isset($params['backup_id']))
                {
                    ...
                    update_option('wpvivid_export_auto_migration_finish', '1');
                }
                $ret['result'] = WPVIVID_SUCCESS;
                echo json_encode($ret);
            }
