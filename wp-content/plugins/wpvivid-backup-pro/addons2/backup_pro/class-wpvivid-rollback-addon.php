...
class WPvivid_Rollback_Addon
{
    ...
    public function init_rollback_setting()
    {
        ...
        if(version_compare($old_rollback_version, '1.0.1', '<'))
        {
            ...
            {
                update_option('wpvivid_rollback_retain_local', 0);
            }
            else
            {
                ...
                {
                    update_option('wpvivid_rollback_retain_local', 1);
                }
            }
            update_option('wpvivid_rollback_version', '1.0.1');
        }
    }
                    ...
                    if($version_data['upload']===false)
                    {
                        ...
                        if(file_exists($file_name))
                        {
                            ...
                            if($ret['result']=='success')
                            {
                                ...
                                update_option("wpvivid_rollback_plugin_data",$rollback_data);
        ...
        if(!empty($rollback_data))
        {
            foreach ($rollback_data as $slug=>$data)
            {
                ...
                foreach ($data['version'] as $version=>$version_data)
                {
                    if($version_data['upload']===false)
                    {
                        $path=WP_CONTENT_DIR.'/'.WPvivid_Setting::get_backupdir().'/rollback/themes/'.$slug.'/'.$version;
                        $file_name=$path.'/'.$slug.'.zip';
                        if(file_exists($file_name))
                        {
                            ...
                            if($ret['result']=='success')
                            {
                                ...
                                $rollback_data[$slug]['version'][$version]['file']['modified']=filemtime($file_name);
                                $rollback_data[$slug]['version'][$version]['file']['file_name']=basename($file_name);
                                update_option("wpvivid_rollback_theme_data",$rollback_data);
                                if(!$rollback_retain_local)
                                {
                                    @unlink($file_name);
                                }
        ...
        else
        {
            if($backup_content === 'plugin')
            {
                ...
                if(isset($backup_slug) && !empty($backup_slug))
                {
                    ...
                    update_option('wpvivid_send_mail_plugin_option', $send_mail_plugin_option);
            ...
            if($backup_content === 'theme')
            {
                ...
                if(isset($backup_slug) && !empty($backup_slug))
                {
                    ...
                    update_option('wpvivid_send_mail_theme_option', $send_mail_theme_option);
    ...
    public function send_mail_plugin_update_event()
    {
        ...
        if(!empty($send_mail_plugin_option))
        {
            ...
            update_option('wpvivid_send_mail_plugin_option', array());
        }
    }
    ...
    public function send_mail_theme_update_event()
    {
        ...
        if(!empty($send_mail_theme_option))
        {
            ...
            update_option('wpvivid_send_mail_theme_option', array());
        }
    }
    ...
    public function check_plugins_versions()
    {
        ...
        $counts=get_option('wpvivid_max_rollback_count',array());
        $rollback_retain_local=get_option('wpvivid_rollback_retain_local',0);
        $path=WP_CONTENT_DIR.'/'.WPvivid_Setting::get_backupdir().'/'.'rollback/plugins/';
            ...
            foreach ($rollback_version as $version)
            {
                ...
                if(!empty($plugin['rollback_data']))
                {
                    if(isset($plugin['rollback_data']['version'][$version]))
                    {
                        ...
                        update_option('wpvivid_rollback_plugin_data',$rollback_plugin_data);
                    }
                }
            ...
            foreach ($rollback_version as $version)
            {
                ...
                if(!empty($theme_data['rollback_data']))
                {
                    if(isset($theme_data['rollback_data']['version'][$version]))
                    {
                        ...
                        update_option('wpvivid_rollback_theme_data',$rollback_theme_data);
                    }
                }
    ...
    public function backup($response, $args)
    {
        ...
        if(!empty($plugin))
        {
            ...
            if($slug=='.')
            {
                ...
                $parent_path=WP_CONTENT_DIR.'/'.WPvivid_Setting::get_backupdir().'/rollback/plugins';
                $path=WP_CONTENT_DIR.'/'.WPvivid_Setting::get_backupdir().'/rollback/plugins/'.$slug.'/'.$version;
                $file_name=$slug.'zip';
            }
            else
        ...
        if(!empty($theme))
        {
            if($this->get_theme_enable_auto_backup_status($theme))
            {
                ...
                $parent_path=WP_CONTENT_DIR.'/'.WPvivid_Setting::get_backupdir().'/rollback/themes';
    ...
    public function set_rollback_setting()
    {
        ...
        update_option('wpvivid_max_rollback_count',$counts);
        update_option('wpvivid_auto_backup_db_before_update', $auto_backup_before_update);
        update_option('wpvivid_rollback_retain_local', $rollback_retain_local);
        update_option('wpvivid_rollback_remote', $rollback_remote);
        update_option('wpvivid_rollback_remote_id', $remote_id);
        ...
        if (isset($_POST['versions']) && !empty($_POST['versions']) && is_array($_POST['versions']))
        {
            ...
            foreach ($versions as $version)
            {
                ...
                else if($type=="themes")
                {
                    ...
                    unset($rollback_plugin_data[$slug]['version'][$version]);
                    update_option('wpvivid_rollback_theme_data',$rollback_plugin_data);
                }
            }
    ...
    public function prepare_rollback_file()
    {
        ...
        $path=WP_CONTENT_DIR.'/'.WPvivid_Setting::get_backupdir().'/';
        ...
        else
        {
            ...
            if($data['upload'])
            {
                ...
                if($rollback_remote)
                {
                    ...
                    update_option("wpvivid_rollback_download_info",$download_info);
    ...
    public function rollback_plugins_sync()
    {
        ...
        if($result['result']=='success')
        {
            ...
            update_option("wpvivid_tmp_rollback_plugin_data",$rollback_data);
    ...
    public function rollback_plugins_sync_continue()
    {
        ...
        {
            update_option("wpvivid_rollback_plugin_data",array());
            $ret['result']='success';
            echo json_encode($ret);
            die();
        }
        ...
        foreach ($tmp_rollback_data as $slug=>$data)
        {
            ...
            update_option('wpvivid_tmp_rollback_plugin_data',$tmp_rollback_data);
    ...
    public function rollback_themes_sync()
    {
        ...
        if($result['result']=='success')
        {
            ...
            update_option("wpvivid_tmp_rollback_themes_data",$rollback_data);
    ...
    public function rollback_themes_sync_continue()
    {
        ...
        if(empty($tmp_rollback_data))
        {
            update_option("wpvivid_rollback_theme_data",array());
            $ret['result']='success';
            echo json_encode($ret);
            die();
        }
        ...
        foreach ($tmp_rollback_data as $slug=>$data)
        {
            ...
            update_option('wpvivid_tmp_rollback_themes_data',$tmp_rollback_data);
