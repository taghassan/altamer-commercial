...
class WPvivid_B2_addon extends WPvivid_Remote_addon
{
    ...
    private function _upload($task_id,$local_file,$remote_path,$callback)
    {
        ...
        if($file_size<1024*1024*5)
        {
            ...
            if($ret['result']!='success')
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('Uploaded small file get upload url failed.','notice');
                return $ret;
            }
        ...
        {
            $wpvivid_plugin->wpvivid_log->WriteLog('Start upload large file.','notice');
            ...
            $wpvivid_plugin->wpvivid_log->WriteLog('Get upload part url.','notice');
            ...
            if($ret['result']!='success')
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('upload_resume failed: '.json_encode($ret),'notice');
                return $ret;
            }

            $wpvivid_plugin->wpvivid_log->WriteLog('Finish large file upload.','notice');

            $ret=$this->finish_large_file($fileId,$ret['sha1_of_parts']);

            if($ret['result']!='success')
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('finish_large_file failed: '.json_encode($ret),'notice');
                return $ret;
            }

            $upload_job=WPvivid_taskmanager::get_backup_sub_task_progress($task_id,'upload',$this->options['id']);
            $upload_job['job_data'][basename($local_file)]['uploaded']=1;
            $wpvivid_plugin->wpvivid_log->WriteLog('Finished uploading '.basename($local_file),'notice');
    ...
    public function finish_large_file($file_id,$sha1_of_parts)
    {
        ...
        global $wpvivid_plugin;
        $wpvivid_plugin->wpvivid_log->WriteLog('remote_post headers: '.json_encode($headers).', post_fields: '.$post_fields.', api_url: '.$api_url, 'notice');
        $ret=$this->remote_post('b2_finish_large_file',$headers,$post_fields,60,$api_url);
        $wpvivid_plugin->wpvivid_log->WriteLog('remote_post result: '.json_encode($ret), 'notice');
        if($ret['result']=='success')
        {
            return $ret;
        }
    ...
    public function get_file_info($fileId)
    {
        ...
        $headers['Authorization'] = $auth_token;
        global $wpvivid_plugin;
        $wpvivid_plugin->wpvivid_log->WriteLog('get_file_info remote_post headers: '.json_encode($headers).', post_fields: '.$post_fields.', api_url: '.$api_url, 'notice');
        $ret=$this->remote_post('b2_get_file_info',$headers,$post_fields,30,$api_url);
        $wpvivid_plugin->wpvivid_log->WriteLog('get_file_info remote_post result: '.json_encode($ret), 'notice');
        if($ret['result']=='success')
        {
            return $ret;
        }
    ...
    public function delete_exist_file($path)
    {
        ...
        $auth_token =   $this->options['authorizationToken'];
        $bucket_id =  $this->options['bucketId'];
        $data = array("bucketId" => $bucket_id,'prefix'=>$path,'startFileName'=>basename($path));
        $post_fields = json_encode($data);
        $headers['Authorization'] = $auth_token;
    ...
    public function upload_small_file($file,$remote_path,$task_id)
    {
        ...
        while($retry<15)
        {
            ...
            $wpvivid_plugin->wpvivid_log->WriteLog('failed ret:'.json_encode($ret),'notice');
            $wpvivid_plugin->wpvivid_log->WriteLog('upload_small_file_loop trys:'.$retry,'notice');
            $retry++;
    ...
    public function upload_resume($file_name,$task_id,$callback)
    {
        ...
        while($total_bytes_sent < $local_file_size)
        {
            ...
            while($retry<15)
            {
                ...
                $wpvivid_plugin->wpvivid_log->WriteLog('failed ret:'.json_encode($ret),'notice');
                $wpvivid_plugin->wpvivid_log->WriteLog('part_no trys:'.$retry,'notice');
                $retry++;
                if(isset($ret['error']) && $ret['error'] == 'Service Unavailable')
                {
                    sleep(30);
            ...
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('upload failed after try times:'.$retry,'notice');
                return $ret;
            }
    ...
    public function upload_loop($file_handle,$total_bytes_sent,$bytes_sent_for_part,$part_no)
    {
        ...
        global $wpvivid_plugin;
        //$wpvivid_plugin->wpvivid_log->WriteLog('upload_loop headers: '.json_encode($headers).', api_url: '.$api_url, 'notice');
    ...
    private function remote_post($method,$headers,$content,$timeout=30,$url='')
    {
        ...
        if(!is_wp_error($response))
        {
            ...
            {
                global $wpvivid_plugin;
                $wpvivid_plugin->wpvivid_log->WriteLog('remote_post error result: '.json_encode($response), 'notice');
        ...
        {
            global $wpvivid_plugin;
            $wpvivid_plugin->wpvivid_log->WriteLog('remote_post_error: '.json_encode($response),'notice');
