...
class WPvivid_Dropbox_addon extends WPvivid_Remote_addon
{
    ...
    private function _put($task_id,$dropbox,$file,$callback)
    {
        ...
        if($this -> current_file_size > $this -> upload_chunk_size)
        {
            ...
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('Upload file size: '.$this -> current_file_size,'notice');
            ...
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('Upload file continue.','notice');
    ...
    public function large_file_upload($task_id,$session_id,$file,$dropbox,$callback){
        ...
        try
        {
            while($data =fread($fh,$this -> upload_chunk_size))
            {
                ...
                {
                    $wpvivid_plugin->wpvivid_log->WriteLog('Upload loop result: '.json_encode($ret),'notice');
                    return $ret;
                }
            ...
            $wpvivid_plugin->wpvivid_log->WriteLog('Upload upload_session_finish start.','notice');
            $result = $dropbox -> upload_session_finish($session_id,$offset,$path);
            $wpvivid_plugin->wpvivid_log->WriteLog('Upload upload_session_finish finish.','notice');
            if(isset($result['error_summary']))
            {
            ...
            if($ret['result'] === WPVIVID_PRO_SUCCESS)
            {
                ...
                if(isset($result['error_summary']))
                {
                    $wpvivid_plugin->wpvivid_log->WriteLog('Upload upload_session_finish error: '.json_encode($result),'notice');
                    $ret = array('result' => WPVIVID_PRO_FAILED,'error' => $result['error_summary']);
                }else{
                    $ret = array('result'=> WPVIVID_PRO_SUCCESS);
                }
        ...
        catch (Exception $error)
        {
            ...
            $wpvivid_plugin->wpvivid_log->WriteLog('Upload file exception: '.$message,'notice');
    ...
    public function chunk_download($download_info,$callback)
    {
        try
        {
            ...
            $options = $this->options;
            $path = '/'.$options['path'].'/'.$download_info['file_name'];
    ...
    public function upload_rollback($file,$folder,$slug,$version)
    {
        ...
        $options = $this -> options;
        $path = '/'.$options['path'].'/rollback_ex/'.$folder.'/'.$slug.'/'.$version.'/'.basename($file);
    ...
    public function download_rollback($download_info)
    {
        ...
        $options = $this->options;
        $path = '/'.$options['path'].'/rollback_ex/'.$type.'/'.$slug.'/'.$version.'/'.$download_info['file_name'];
    ...
    public function cleanup_rollback($type,$slug,$version)
    {
        ...
        $path = '/'.$options['path'].'/rollback_ex/'.$type.'/'.$slug.'/'.$version.'/'.$slug.'.zip';
        $dropbox -> delete($path);
        return array('result'=>WPVIVID_PRO_SUCCESS);
    }
    ...
    {
        $ret=$this->_scan_child_folder_backup($this->options['path'],$incremental_path,$dropbox);
        if($ret['result']==WPVIVID_PRO_SUCCESS)
        {
    ...
    public function get_backup_info($backup_info_file,$folder_type,$incremental_path='')
    {
        try
        {
            ...
            {
                $path = '/'.$options['path'].'/'.$backup_info_file;
            }
            else if($folder_type=='Migrate')
            {
            ...
            {
                $path='/'.$this->options['path'].'/rollback/'.$backup_info_file;
            }
            else if($folder_type=='Incremental')
            {
    ...
    public function scan_rollback($type)
    {
        ...
        if($type === 'plugins')
        {
            $path=$options['path'].'/rollback_ex/plugins';
        ...
        {
            $path=$options['path'].'/rollback_ex/themes';
    ...
    public function get_rollback_data($type,$slug)
    {
        ...
        if($type === 'plugins')
        {
            $path=$options['path'].'/rollback_ex/plugins/'.$slug;
            $response=$this->_scan_folder($path,$dropbox);
        ...
        {
            $path=$options['path'].'/rollback_ex/themes/'.$slug;
            $response=$this->_scan_folder($path,$dropbox);
