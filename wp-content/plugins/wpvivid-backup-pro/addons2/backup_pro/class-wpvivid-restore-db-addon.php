...
class WPvivid_Restore_DB_Addon
{
    public $log;
    public $db_method;
    ...
    public function restore($sub_task,$backup_id)
    {
        ...
        if($sub_task['unzip_file']['unzip_finished']==0)
        {
            ...
            if(isset($files[0]['options']['is_crypt'])&&$files[0]['options']['is_crypt']=='1' || $is_crypt === true)
            {
                ...
                {
                    $temp_sql_file=$backup_item->get_backup_path($file['file_name']);
                    $sql_files=$this->get_sql_file($temp_sql_file);
                    foreach ($sql_files as $tmp_sql_file)
                    {
                        $this->log->WriteLog('Decrypting sql file:'.$tmp_sql_file['file_name'],'notice');
                        $ret=$this->restore_crypt_db($tmp_sql_file['file_name'],$local_path);
                        if($ret['result']!='success')
                        {
                            return $ret;
                        }
                        ...
                        {
                            $sql_file['sql_file_name']=$ret['sql_file'];
                            $sql_file['sql_file_size']=filesize($local_path.$ret['sql_file']);
                            $sql_file['sql_offset']=0;
                            $sql_file['finished']=0;
                            $sub_task['exec_sql']['sql_files'][$sql_file['sql_file_name']]=$sql_file;
                            @unlink($local_path.$tmp_sql_file['file_name']);
                        }
                    }
                }
        ...
        {
            $ret=$this->restore_additional_db($sql_files,$local_path,$sub_task);
        }
        else
        {
    ...
    public function check_is_sql_file($file_path)
    {
        $sql_handle = fopen($file_path,'r');
        if($sql_handle===false)
        {
            $ret['result']=WPVIVID_PRO_FAILED;
            $ret['error']='File not found. file name:'.$file_path;
            return $ret;
        }
        ...
        while(!feof($sql_handle))
        {
            ...
            if (empty($line) || $startWith == '--' || ($startWith == '/*'&&$startWithEx!='/*!') || $startWith == '//')
            {
                ...
                {
                    $matcher = array();
                    if(preg_match('# site_url: (.*?) #',$line,$matcher))
                    {
                        $find_site_url=true;
                    }
                }
        ...
        return $find_site_url;
    }

    public function restore_crypt_db($file,$local_path)
    {
        $restore_task=get_option('wpvivid_restore_task',array());
        $restore_detail_options=$restore_task['restore_detail_options'];
        $password=isset($restore_detail_options['restore_db_password'])?$restore_detail_options['restore_db_password']:false;
        ...
        if($ret['result']=='success')
        {
            ...
            {
                unset($archive);
                if(!$this->check_is_sql_file($file_path))
                {
                    $ret['result']=WPVIVID_PRO_FAILED;
                    $ret['error']='Failed to unzip the file. Maybe the password is incorrect. Please check your password and try again.';
                    return $ret;
                }
                $new_file_path=str_replace('.decrypted.zip','',$file_path);
                rename($file_path,$new_file_path);
                $sql_file=$new_file_path;
                $ret['result']=WPVIVID_SUCCESS;
                $ret['sql_file']=basename($sql_file);
                return $ret;
            }
            else
            {
                $ret= $this->extract($file_path,$local_path);
                if($ret['result']!=='success')
                {
                    $ret['error']='Failed to unzip the file. Maybe the password is incorrect. Please check your password and try again.';
                    return $ret;
    ...
    public function restore_additional_db($sql_files,$local_path,$sub_task)
    {
        $ret['result']='success';
        $ret['sub_task']=$sub_task;
        ...
        foreach ($sql_files as $sql_file_name=>$sql_file)
        {
            ...
            $this->update_sub_task($sub_task);
            $ret=$this->execute_extra_sql_file($sql_file_name,$local_path,$sub_task);
            if($ret['result']=='success')
            {
                $sub_task=$ret['sub_task'];
                break;
    ...
    public function execute_extra_sql_file($sql_file,$local_path,$sub_task)
    {
        $options=$sub_task['options'];

        $dbhost = $options['host'];
        $dbuser = $options['user'];
        $dbpassword = $options['pw'];
        $dbname = $options['db'];

        $restore_extra_db=new WPvivid_Restore_Extra_DB_Addon($this->log);
        $restore_extra_db->set_db_info($dbhost, $dbuser, $dbpassword, $dbname);
        return $restore_extra_db->execute_extra_sql_file($sql_file,$local_path,$sub_task);
    }
