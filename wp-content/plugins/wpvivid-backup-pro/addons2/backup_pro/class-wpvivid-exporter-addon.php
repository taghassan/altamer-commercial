...
class WPvivid_Post_List_Ex extends WP_List_Table
{
    public $post_ids;
    public $page_num;
    ...
    protected function display_tablenav( $which ) {
        ...
        <div class="tablenav <?php echo esc_attr( $which ); ?>" style="<?php esc_attr_e($css_type); ?>">
            <div class="alignleft actions bulkactions">
                <label>
                    <?php echo '<input type="checkbox" option="export" name="all_page">'; ?>
                    <span><strong><?php _e('Export all pages', 'wpvivid'); ?></strong></span>
                </label>
...
class WPvivid_Exporter_taskmanager_Ex
{
...
class WPvivid_Exporter_task_Ex
{
    private $task;
    ...
    public function get_next_posts()
    {
        ...
        $next_post_ids=$post_ids;
        $this->task['options']['backup_options']['post_ids']=array();
...
class WPvivid_Exporter_Item_Ex{
    private $config;
...
class WPvivid_Exporter_Ex
{
    ...
    public function __construct($task_id=false,$task=false)
    {
        if($task_id!==false)
        {
            $this->task=new WPvivid_Exporter_task_Ex($task_id);
        }
        else if($task!==false)
        {
            $this->task=new WPvivid_Exporter_task_Ex(false,$task);
        }
        else
        {
            $this->task=new WPvivid_Exporter_task_Ex();
        }
    }
    ...
    public function export($task_id)
    {
        ...
        while($next!==false)
        {
            ...
            $wpvivid_plugin->wpvivid_log->WriteLog('Size:'.sizeof($next['next_post_ids']),'notice');
    ...
    public function export_post_to_xml($posts_ids,$attachment_ids,$file_name,$export_type)
    {
        ...
        //$files=array();
        $export_folder = WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPvivid_Setting::get_backupdir().DIRECTORY_SEPARATOR.WPVIVID_IMPORT_EXPORT_DIR_ADDON;
    ...
    private function zip_media_files($xml_file,$files,$file_name,$export_type,$json_info=false)
    {
        ...
        $path=WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPvivid_Setting::get_backupdir().DIRECTORY_SEPARATOR.WPVIVID_IMPORT_EXPORT_DIR_ADDON.DIRECTORY_SEPARATOR.$file_name.'_export_'.$export_type.'.zip';
        $options['compress']['no_compress']=1;
        $options['compress']['use_temp_file']=1;
        $options['compress']['use_temp_size']=16;
        $options['root_flag']=WPVIVID_PRO_BACKUP_ROOT_WP_CONTENT;
