...
class WPvivid_pCloud_addon extends WPvivid_Remote_addon
{
    ...
    public function _upload($task_id,$local_file,$folder_id,$callback)
    {
        ...
        $wpvivid_plugin->wpvivid_log->WriteLog('get upload id success:'.json_encode($ret),'notice');
        ...
        while (!feof($file))
        {
            ...
            while($retry<5)
            {
                //$wpvivid_plugin->wpvivid_log->WriteLog('offset:'.$params["uploadoffset"],'notice');
                $ret=$this->upload_chunk($params, $content);
                if($ret['result']==WPVIVID_PRO_SUCCESS)
                {
                    break;
                }
                $wpvivid_plugin->wpvivid_log->WriteLog('offset trys:'.$retry,'notice');
                $retry++;
            }
    ...
    public function createUpload()
    {
        ...
        if($response['result']==WPVIVID_PRO_SUCCESS)
        {
            ...
            {
                $wpvivid_plugin->wpvivid_log->WriteLog('isset not have upload_id.','notice');
                $ret['result']=WPVIVID_PRO_FAILED;
        ...
        {
            $wpvivid_plugin->wpvivid_log->WriteLog('remote_get failed.','notice');
            return $response;
        }
    }
    ...
    private function remote_put($method,$params,$content,$timeout=30)
    {
        ...
        if(!is_wp_error($response))
        {
            ...
            if($body!=null)
            {
                ...
                else
                {
                    $ret['result']=WPVIVID_PRO_FAILED;
                    $ret['error']= $body['error'];
                    $wpvivid_plugin->wpvivid_log->WriteLog('body error:'.json_encode($body),'notice');
                    return $ret;
                }
            }
            else
            {
                $ret['result']=WPVIVID_PRO_FAILED;
                $ret['error']= $response;
                $wpvivid_plugin->wpvivid_log->WriteLog('response error:'.$ret['error'],'notice');
                return $ret;
            }
        }
        else
        {
            $ret['result']=WPVIVID_PRO_FAILED;
            $ret['error']=$response->get_error_message();
            $wpvivid_plugin->wpvivid_log->WriteLog('remote put failed:'.$ret['error'],'notice');
            return $ret;
        }
    }
    ...
    public function get_backup_info($backup_info_file,$folder_type,$incremental_path='')
    {
        ...
        {
            $local_path = WP_CONTENT_DIR .'/'. $local_setting['path'] .'/'.$backup_info_file;
        }
        else {
            $local_path = WP_CONTENT_DIR .'/wpvividbackups/'. $backup_info_file;
        }
        @unlink($local_path);
