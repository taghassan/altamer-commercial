	...
	class MFN_Options
	{
		...
		public function _validate_options($plugin_options)
		{
			...
			if (! empty($plugin_options['import'])) {
				...
				$imported_options = @unserialize(trim($import, '###'), ['allowed_classes' => false]);
      ...
      foreach ( (array) $wp_settings_sections[ $page ] as $section ) {
		...
		function _options_page_html(){
			...
			if( $tmp_header_id = get_option('mfn_header_entire_site') ){
				if( 'publish' == get_post_status($tmp_header_id) ){

					$form_class_header = 'template-header';

					// skip if there are any exclusions

					$conditions = get_post_meta($tmp_header_id, 'mfn_template_conditions', true);
					if( $conditions ){
						$conditions = json_decode($conditions, true);

						foreach( $conditions as $condition ){
							if( !empty($condition['rule']) && 'exclude' == $condition['rule'] ){
								$form_class_header = '';
							}
						}
					}

					$form_class .= ' '. $form_class_header;
			...
			if( $tmp_footer_id = get_option('mfn_footer_entire_site') ){

				if( 'publish' == get_post_status($tmp_footer_id) ){

					$form_class_footer = 'template-footer';

					// skip if there are any exclusions

					$conditions = get_post_meta($tmp_footer_id, 'mfn_template_conditions', true);
					if( $conditions ){
						$conditions = json_decode($conditions, true);
				...
				echo '<form method="post" action="options.php" enctype="multipart/form-data" >';
				  ...
				  echo '</div>';
				    ...
				    echo '<div class="mfn-content">';
							...
							if( false !== strpos($form_class, 'template-header template-footer') ){
								echo '<div class="mfn-alert">';
									echo '<div class="alert-icon mfn-icon-information"></div>';
									echo '<div class="alert-content">';
										echo '<p style="margin-bottom:0"><a target="_blank" href="edit.php?post_type=template&tab=header">Header Builder</a> is <b>active</b>. Header related options are disabled.</p>';
										echo '<p><a target="_blank" href="edit.php?post_type=template&tab=footer">Footer Builder</a> is <b>active</b>. Footer related options are disabled.</p>';
									echo '</div>';
								echo '</div>';
							} elseif ( false !== strpos($form_class, 'template-header') ){
								echo '<div class="mfn-alert">';
									echo '<div class="alert-icon mfn-icon-information"></div>';
									echo '<div class="alert-content">';
										echo '<p><a target="_blank" href="edit.php?post_type=template&tab=header">Header Builder</a> is <b>active</b>. Header related options are disabled.</p>';
									echo '</div>';
								echo '</div>';
							} elseif ( false !== strpos($form_class, 'template-footer') ){
								echo '<div class="mfn-alert">';
									echo '<div class="alert-icon mfn-icon-information"></div>';
									echo '<div class="alert-content">';
										echo '<p><a target="_blank" href="edit.php?post_type=template&tab=footer">Footer Builder</a> is <b>active</b>. Footer related options are disabled.</p>';
