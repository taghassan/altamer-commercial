...
function loopAllStyleFields(uid = false){
    ...
    if( _.isEmpty(copied[0]) ) copied[0] = mfnvbvars.page_options;
...
let historyStorage = {
    ...
    add: function() {
        ...
        //console.log('add history '+historyStorage.add.caller);
        ...
        let new_hist = {
            'obj': mfnvbvars.pagedata,
            'form': prepareForm.get(),
            'pageid': pageid,
            'uid': edited_item ? edited_item.uid : false
        };

        if( !$('body').hasClass('mfn-history-ajax-mode') ) new_hist['html'] = $builder.html();

        history.unshift(new_hist);
         ...
         $('.mfn-history-btn').on('click', function(e) {
            ...
            if( !$el.hasClass('inactive') && !$el.hasClass('loading') ){
                ...
                if( $el.hasClass('btn-redo') && historyStorage.i > 0 ){
                    historyAction = 'redo';
                    historyStorage.i--;
                }else if( $el.hasClass('btn-undo') && historyStorage.i <= history.length ){
                    historyStorage.i++;
                }else{
    ...
    index: function() {
        ...
        if(historyStorage.i < history.length-1){
    ...
    restore: function() {
        let history = historyStorage.object();
        var h_event = history[historyStorage.i];
...
$editpanel.on('click', '.btn-save-changes', function(e){
    ...
    if( $('.modal-display-conditions').length && !$('.modal-display-conditions').hasClass('show') && ( builder_type == 'popup' || builder_type == 'header' || builder_type == 'shop-archive' || builder_type == 'single-product' || builder_type == 'footer' || builder_type == 'single-post' || builder_type == 'single-portfolio' ) ){
...
var prepareForm2 = {
    ...
    get: function( uid = false ){
        ...
        let post_id = samplecontentid ? samplecontentid : pageid;
        ...
        $wrapper.each(function() {
            ...
            if( typeof this_vb['attr'] !== 'undefined' ) {
                ...
                this_vb['attr']['vb_postid'] = $(this).closest('.mfn-queryloop-item-wrapper').length ? $(this).closest('.mfn-queryloop-item-wrapper').attr('data-post') : post_id;
        ...
        $wrapper.find( '.vb-item.mcb-column' ).each(function() {
            ...
            if( !items.filter( (item) => item.uid === col_uid ).length ){
                ...
                if( typeof this_col['attr'] !== 'undefined' ) {
                    ...
                    this_col['attr']['vb_postid'] = $(this).closest('.mfn-queryloop-item-wrapper').length ? $(this).closest('.mfn-queryloop-item-wrapper').attr('data-post') : post_id;
                }
    ...
    get: function(uid = false) {
        ...
        $wrapper.each(function(s) {
            ...
            if( this_vb && typeof this_vb.uid !== 'undefined' ){
                ...
                if( $(this).find('.wrap.vb-item').length ){
                    this_vb.wraps = [];
                    prepareForm.wraps( $(this).children('.section_wrapper') );
                }

                prepareForm.object.push(this_vb);
            }
        ...
        $wraps.each(function(w) {
            ...
            if( this_wrap ) {
                ...
                if( !prepareForm.formData[prepareForm.si]['wraps'].filter( (item) => item.uid === wrap_uid ).length ){
                    ...
                    prepareForm.object.push(this_wrap);
                }
        ...
        item: function() {
            ...
            if($builder.find('.mcb-item-'+be_layout.drag_new.container).length){
                ...
                if(be_layout.drag_new.pos == 'before'){
                    $builder.find('.mcb-item-'+be_layout.drag_new.container).before(new_element);
                }else{
                    $builder.find('.mcb-item-'+be_layout.drag_new.container).after(new_element);
                }

            }else{

                if(be_layout.drag_new.pos == 'before'){
                    $builder.find('.mcb-wrap-'+be_layout.drag_new.container+' > .mcb-wrap-inner').prepend(new_element);
                }else{
                    $builder.find('.mcb-wrap-'+be_layout.drag_new.container+' > .mcb-wrap-inner').append(new_element);
                }
            ...
            setTimeout(function() {
                ...
                if( $builder.find('.vb-item.mcb-wrap-'+be_layout.drag_new.container).closest('.mfn-looped-items').length ){
                    re_render( $builder.find('.vb-item.mcb-wrap-'+be_layout.drag_new.container).closest('.mfn-looped-items').attr('data-uid') );
                }

                if( $builder.find('.vb-item.mcb-wrap-'+be_layout.drag_new.container+'.mfn-looped-items').length ){
                    re_render( $builder.find('.vb-item.mcb-wrap-'+be_layout.drag_new.container+'.mfn-looped-items').attr('data-uid') );
                }

                if( $builder.find('.vb-item.mcb-item-'+be_layout.drag_new.container).closest('.mfn-looped-items').length ){
                    re_render( $builder.find('.vb-item.mcb-item-'+be_layout.drag_new.container).closest('.mfn-looped-items').attr('data-uid') );
                }
    ...
    sortable: {
        ...
        modules: function() { 
            $builder.find('.mfn-wrapper-for-wraps').sortable({
                ...
                start: function(event, elem) {
                    if( elem.item.hasClass('mcb-wrap') ) stickyWrap.reset();
                ...
                stop: function(event, elem) {
                    ...
                    $builder.find('.mfn-current-dragover').removeClass('mfn-current-dragover');
                    if( elem.item.hasClass('mcb-wrap') ) stickyWrap.init();
...
$editpanel.on("click", 'div:not(.panel-settings) .segmented-options.single-segmented-option li a', function(e) {
    ...
    if( $editbox.hasClass('image image_height') ){
        ...
        if( $content.find('.'+it+' .mfn-coverimg').length ){
            ...
            $content.find('.'+it+' .mfn-coverimg-wrapper').removeClass('mfn-coverimg-wrapper');
        }
        
        if( val == 'custom' ) {
            $content.find('.'+it+' .image_frame').addClass('mfn-coverimg');
            if( $content.find('.'+it+' .image_wrapper > a').length ){
                $content.find('.'+it+' .image_wrapper > a').addClass('mfn-coverimg-wrapper');
            }else{
...
function onOpenEditForm() {
    ...
    * */
    ...
    /*if( $editpanel.find('.panel-edit-item .mfn-element-fields-wrapper .mfn-toggled.mfn-is-modified:not(.toggled_header)').length ) {
        $editpanel.find('.panel-edit-item .mfn-element-fields-wrapper .mfn-toggled.mfn-is-modified:not(.toggled_header)').each(function() {
            let header = $(this).attr('data-header');
            if( !$(header).hasClass('mfn-is-modified') ) $(header).addClass('mfn-is-modified');
        });
    }*/
    ...
    * */
    ...
    /*$editpanel.find('.mfn-form .mfn-form-row .color-picker .form-control .color-picker-vb').each(function() {
        if( $(this).val() != '' ){
        ...
        if( $copy_el.find('.vb-item').length ) {
            $copy_el.find('.vb-item').each(function() {
                ...
                if( $copy_el.hasClass('mcb-column') ){
                    ...
                    $copy_el.removeClass('mcb-wrap-'+old_col_uid).addClass('mcb-wrap-'+new_col_uid).attr('data-uid', new_col_uid);
                    $copy_el.find('.mcb-wrap-inner').removeClass('mcb-wrap-inner-'+old_col_uid).addClass('mcb-wrap-inner-'+new_col_uid);
                }
...
function addLocalStyle(u, v, s, r, uid){
    ...
    if( r == 'laptop' ){
        $content.find('head').append('<style class="mfn-local-style '+el+r+'">@media(max-width: 1440px){html '+selector_string.replace('.mfn-header-tmpl', '.mfn-ui.mfn-header-tmpl')+' { '+(s.replace('_laptop', ''))+': '+v+' }}</style>');
    }else if( r == 'tablet' ){
        $content.find('body').prepend('<style class="mfn-local-style '+el+r+'">@media(max-width: 959px){html '+selector_string.replace('.mfn-header-tmpl', '.mfn-ui.mfn-header-tmpl')+' { '+(s.replace('_tablet', ''))+': '+v+' }}</style>');
    }else if( r == 'mobile' ){
        ...
        $content.find('head').prepend('<style class="mfn-local-style '+el+r+'">html '+selector_string.replace('.mfn-header-tmpl', '.mfn-ui.mfn-header-tmpl')+' { '+s+': '+v+' }</style>');
    }
...
var openEditForm = {
    ...
    do: function($edited_div, scroll = false) {
        ...
        //console.log('Open form '+openEditForm.do.caller);
            ...
            if( edited_item.jsclass == 'wrap' ){
                replaced_values.wrap.map(function(it) {
                    if( typeof edited_item.attr[it.key] !== 'undefined' && edited_item.attr[it.key].includes(it.old) ) {
                        edited_item.attr[it.key] = it.new;
                    }
                });
        ...
        let ls = JSON.parse(localStorage.getItem('mfnhistory'));
        if( !ls[0].uid ){
            ls[0].uid = edited_item.uid;
                        ...
                        if( $wrap.hasClass('divider') ) {
                            ...
                            if( !$wrap.hasClass('mfn-global-wrap') ){
                                ...
                                if( $wrap.find('.mfn-module.vb-item').length ) {
                                    ...
                                    $wrap.children('.mcb-wrap-inner').children('.mfn-module.vb-item').each(function() {
                                        let $item = $(this);
...
function runAjaxElements(){
    ...
    if($content && $builder.find('.mfn-new-item .mfn-element-edit').length){
        ...
        if( !$builder.find('.mfn-new-item').hasClass('column_placeholder') ) $builder.find('.mfn-new-item > .mfn-header > .mfn-element-edit').trigger('click');
...
init: function(){
  ...
  $content.find('.mcb-wrap.sticky.sticky-'+screen+':not(.stickied)').each(function(){
    ...
    if( screen == 'laptop' && $wrap.hasClass('laptop-one') ) return;
...
function grabFieldStyle($field){
    ...
    if( (cssstyle == 'background-image' || cssstyle == 'background-image_laptop' || cssstyle == 'background-image_tablet' || cssstyle == 'background-image_mobile' || cssstyle == '-webkit-mask-image' ) && val.length && !$box.hasClass('gradient') ){
...
function getFieldChange($field){
    ...
    if ( $editrow.closest('.transform_field').length ){
        ...
        if( $field.val().length ){
            ...
            if( name.includes(':flex') || name.includes(':flex_laptop') || name.includes(':flex_tablet') || name.includes(':flex_mobile') ) editedd_item['attr'][name.replace(':flex', ':max-width')] = $field.val();
        }else if( typeof editedd_item['attr'][name] !== 'undefined' ){
            delete editedd_item['attr'][name];
            if( name.includes(':flex') || name.includes(':flex_laptop') || name.includes(':flex_tablet') || name.includes(':flex_mobile') ) delete editedd_item['attr'][name.replace(':flex', ':max-width')];
...
$.fn.inputDrag = function(options) {
    ...
    var css_path = $(el).closest('.mfn-form-row').attr('data-csspath').replace('mcb-wrap-inner', 'mcb-wrap-inner-'+edited_item.uid);
    ...
    el.mousedown(function(e) {
        ...
        if( $(el).closest('.mfn-form-row').hasClass('margin') || el.closest('.mfn-form-row').hasClass('margin_tablet') || el.closest('.mfn-form-row').hasClass('margin_laptop') || el.closest('.mfn-form-row').hasClass('margin_mobile') ){
            negative = true;
        }
