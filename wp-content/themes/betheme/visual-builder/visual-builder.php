...
if( is_admin() ){
	...
	function mfnvb_gutenberg_functions() {
		...
		if( !in_array($post->post_type, array('page', 'post', 'portfolio', 'template', 'product')) ) return;
...
function mfnvb_updateVbView(){
	...
	if ( $post_type == 'template' ){
		...
		}else if( in_array($tmpl_type, array('single-post', 'single-portfolio')) ){
...
add_action( 'wp_ajax_mfnconverttoglobal', 'mfnvb_converttoglobalsection' );

function mfnvb_converttoglobalsection(){
	...
	update_post_meta($new_id, 'mfn_template_type', 'section');
	update_post_meta( $new_id, 'mfn-page-object', $object );
		
	$mfn_styles = Mfn_Helper::preparePostUpdate(json_decode( stripslashes($object), true), $new_id, 'mfn-page-local-style');
...
function mfnvb_savepostoption(){
	...
	if( $value == '0' && in_array($option, array('mfn_header_template', 'mfn_footer_template')) ){
		...
		$val_to_save = is_array($value) ? json_encode($value) : $value;
		...
		if( strpos($option, 'style:') !== false ){
			...
			$csspath_old = str_replace('postid', $id, $explode[1]);
			$csspath = str_replace('|hover', ':hover', $explode[1]);
