...
function woo_template_body_classes( $classes ) {
	...
	if( empty(mfn_opts_get('shop-list-active-filters')) && empty(mfn_opts_get('shop-list-perpage')) && empty(mfn_opts_get('shop-list-layout')) && empty(mfn_opts_get('shop-list-sorting')) && empty(mfn_opts_get('shop-list-results-count')) ){
...
if ( ! function_exists( 'woocommerce_header_add_to_cart_fragment' ) ) {
	function woocommerce_header_add_to_cart_fragment( $fragments ){
		...
		/*if( empty($mfn_global['header']) && mfn_opts_get('shop-icon-count-if-zero') == 0 && $total == 0){
...
if( mfn_opts_get('shop-list-sorting') == 0 ){
	remove_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 30 );
}
if( mfn_opts_get('shop-list-results-count') == 0 ){
...
function mfn_tell_free_delivery() {
  ...
  if($total < $free):
  	...
  	$diff = round(($free-$total), 2).get_woocommerce_currency_symbol();
...
function mfn_fake_sale_notification() {
  ...
  if( !$return || ( isset($return['date']) && $return['date'] < date('Y-m-d') ) || ( isset($return['type']) && $return['type'] != $type ) || ( isset($return['names']) && $return['names'] != $names_opt ) ) {

  	if( !empty($names_opt) ){
	  	$names = !empty(mfn_opts_get('fake-sale-clients')) ? explode(',', mfn_opts_get('fake-sale-clients')) : array('John','Linda','Ann','Charles','Noah','Lucas','Henry','Camila','Harper','Elizabeth');
	  	$names_length = count($names);
	  }

	  $products = $wpdb->get_results( "SELECT `ID`, `post_title` FROM {$wpdb->prefix}posts WHERE post_type = 'product' and post_status = 'publish' order by RAND() LIMIT {$product_limit}" );

		if( is_iterable($products) ) {
			foreach( $products as $p=>$product ) {
				$string = '<a class="mfn-fake-sale-noti-img" href="'.get_permalink($product->ID).'"><img loading="lazy" src="'.get_the_post_thumbnail_url($product->ID).'" alt=""></a><div class="mfn-fake-sale-noti-desc"><p class="mfn-fake-sale-noti-desc-title">';

				if( $type == '2' ){
					if( $p % 2 == 0 ){
						$string .= ($names ? trim( $names[rand(0, $names_length-1)] ) : $translate['fake-sale-notification-someone']) .' '. $translate['fake-sale-notification-single'] . ' <a href="'.get_permalink($product->ID).'">'.$product->post_title.'</a>';
					}else{
						$string .= __( 'Product', 'woocommerce' ) . ' <a href="'.get_permalink($product->ID).'">'.$product->post_title.'</a> ' .sprintf( $translate['fake-sale-notification-multi'], rand(5, 10) );
					}
				}elseif( $type == '1' ){
					$string .= __( 'Product', 'woocommerce' ) . ' <a href="'.get_permalink($product->ID).'">'.$product->post_title.'</a> ' .sprintf( $translate['fake-sale-notification-multi'], rand(5, 10) );
				}else{
