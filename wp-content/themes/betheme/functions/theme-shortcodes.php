		...
		ob_start();
			if ( ! empty( $product_tabs ) ) :
				echo '<div class="'.implode(' ', $classes).'">
					...
					echo '</ul>';
					foreach ( $product_tabs as $key => $product_tab ) :
						echo '<div class="mfn-woocommerce-tabs-content '.(++$y==1 ? 'active' : '').'" id="tab-'.esc_attr( $key ).'">';
...
if (! function_exists('sc_heading')) {
	function sc_heading($attr, $content = null)
	{
		...
		$output .= '<'.$header_tag.' class="title'. $class.'">';
			if( ! empty($link) ) $output .= '<a class="title_link" href="'. $link .'" '. $target_escaped .' title="'. esc_attr($link_title ?? '') .'">';
				...
				$output .= '<div class="desc_wrapper">';
					...
					if ( $number || $number === '0' ) {
						$output .= '<div class="number-wrapper">';
							...
							$output .= '<span class="number" data-to="'. intval($number, 10) .'" '. $duration .'>'. intval($number, 10) .'</span>';
...
if (! function_exists('sc_image')) {
	function sc_image($attr, $content = null) {
		...
		if ($link || $link_image) {
			...
			if( !empty($attr['image_height']) && $attr['image_height'] == 'custom' ) $coverimg = 'class="mfn-coverimg-wrapper"';
			...
			$output .= '<div class="image_frame element_classes image_item scale-with-grid'. esc_attr($class_div) .'" '. $style_escaped .' role="link" aria-label="Image with links" tabindex="0">';
				...
				$output .= '<div class="image_wrapper">';
					...
					// This variable has been safely escaped above in this function
					$output .= '<a '.$coverimg.' href="'. esc_url(be_dynamic_data($link_all)) .'" '. $rel_escaped .' '. $target_escaped .' tabindex="-1" title="'. esc_attr($link_title ?? '') .'">';
						$output .= '<div class="mask"></div>';
						// This variable has been safely escaped above in this function
						$output .= $image_output;
					$output .= '</a>';
					...
					$output .= '<div class="image_links">';
						...
						if ($link) {
							// This variable has been safely escaped above in this function
							$output .= '<a href="'. esc_url(be_dynamic_data($link)) .'" class="link" '. $target_escaped .' tabindex="-1" title="'. esc_attr($link_title ?? '') .'"><svg viewBox="0 0 26 26" aria-label="'. __('go to link', 'betheme') .'"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><path d="M10.17,8.76l2.12-2.12a5,5,0,0,1,7.07,0h0a5,5,0,0,1,0,7.07l-2.12,2.12" class="path"></path><path d="M15.83,17.24l-2.12,2.12a5,5,0,0,1-7.07,0h0a5,5,0,0,1,0-7.07l2.12-2.12" class="path"></path><line x1="10.17" y1="15.83" x2="15.83" y2="10.17" class="path"></line></g></svg></a>';
						}
					$output .= '</div>';
			...
			if( !empty($attr['image_height']) && $attr['image_height'] == 'custom' ) $coverimg = 'mfn-coverimg-wrapper';
...
if (! function_exists('sc_button')) {
	function sc_button($attr, $content = null)
	{
		...
		if ( $align ) {
			...
			// This variable has been safely escaped above in this function
			$output .= '<a '.implode(' ', $data_tags).' class="button '. esc_attr( $class ) .'" '. $button_id .' href="'. esc_url( $link ) .'"'. $attributes_escaped .' title="'. esc_attr($link_title ?? '') .'">';
		...
		$output = '<div class="column_filters">';
			...
			$output .= '<div class="portfolio_wrapper element_classes isotope_wrapper '. esc_attr($class) .'">';
				...
				$output .= '<ul class="portfolio_group lm_wrapper isotope col-'. intval($columns, 10) .' '. esc_attr($style) .'">';
					$output .= mfn_content_portfolio($query_portfolio, $style, $link);
				$output .= '</ul>';
	...
	function sc_tag_cloud($attr) {
		...
		$output = '<ul class="'.implode(' ', $classes).'">';
			...
			if ( $terms && ! is_wp_error( $terms ) ) {
				foreach ( $terms as $term ) {
					$output .= '<li>';
						$output .= '<a href="' . esc_attr( get_term_link( $term->slug, $category ) ) . '">';
							$output .= __( $term->name );
						$output .= '</a>';
					$output .= '</li>';
				}
...
if (! function_exists('sc_gallery')) {
	function sc_gallery($attr)
	{
		...
		foreach ($attachments as $id => $attachment) {
			...
			$output .= "<{$itemtag} class='gallery-item' data-title='{$image_data[$lightbox_title_src]}' data-description='{$image_data[$lightbox_description_src]}'><div class='gallery-item-wrapper'>";
			$output .= "
