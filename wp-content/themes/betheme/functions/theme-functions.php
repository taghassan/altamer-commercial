	...
	function mfn_ID( $param = false )
	{
		...
		if (function_exists('is_woocommerce') && is_woocommerce() && !is_admin() ) {
			...
			if( is_product() ){
				...
				if( !empty($mfn_global['single_product']) ) return $mfn_global['single_product'];
...
if (! function_exists('mfn_single_post_ID')) {
	function mfn_single_post_ID($type) {
		...
		// conditions
		if( !empty(get_option('mfn_'.$type.'_template')) ){
			$sp_tmpl = get_option('mfn_'.$type.'_template');
	...
	function mfn_template_part_ID( $type, $id = false ) {
		...
		if( $id || is_singular() || is_search() ){
			...
			if( !empty($entire_site) && is_numeric($entire_site) && get_post_status($entire_site) == 'publish' && get_post_type($entire_site) == 'template' ){
				if( !apply_filters('bebuilder_preview', false) || get_post_type($post_id) != 'template' || (get_post_type($post_id) == 'template' && in_array(get_post_meta($post_id, 'mfn-template-type', true), array('shop-archive', 'single-product'))) ){
					return $entire_site;
				}
			}
...
if (! function_exists('mfn_addons_ID')) {
	function mfn_addons_ID( $type, $id = false ) {
		...
		echo 'mfn_'.$type.'_addons_singular'.$langfix;
		echo '<pre>';
		print_r($return);
		echo '</pre>';*/
...
if (! function_exists('mfn_global_sidemenu_id')) {
	function mfn_global_sidemenu_id() {
		global $wpdb;
		$sm = $wpdb->get_row( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'mfn_sidemenu_visibility' AND meta_value = 'always-visible'" );
		if( !empty($sm->post_id) && get_post_status($sm->post_id) == 'publish' && get_post_type($sm->post_id) == 'template' ) return $sm->post_id;
		return false;
	}
}
		...
		if ($type == 'simple') {
			...
			$output .= '<div class="share-simple-wrapper '. esc_attr($class) .'">';
				...
				$output .= '<div class="icons">';
					...
					$output .= '<a target="_blank" class="twitter" href="https://twitter.com/intent/tweet?text='. urlencode( esc_attr(wp_get_document_title()) .'. '. esc_url(get_permalink()) ) .'"><i class="icon-twitter" aria-label="twitter icon"></i></a>';
			...
			if ($load_more) {
				...
				$output .= '<div class="column one pager_wrapper">';
					$output .= '<div class="pager">';
						...
						if ($paged >1) {
							$output .= '<a class="prev_page" href="'. esc_url(get_pagenum_link($prev)) .'"><i class="icon-left-open" aria-hidden="true"></i>'. esc_html($translate['prev']) .'</a>';
						}
						...
						if ($paged < $total) {
							$output .= '<a class="next_page" href="'. esc_url(get_pagenum_link($next)) .'">'. esc_html($translate['next']) .'<i class="icon-right-open" aria-hidden="true"></i></a>';
						}
